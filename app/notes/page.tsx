...
export default function NotesPage() {
  return (
    <div style={{ background: "#06040e" }} className="min-h-screen">
      ...
      {/* Header */}
      <div className="relative overflow-hidden" style={{ borderBottom: "1px solid rgba(180,100,255,0.2)" }}>
        <div className="absolute inset-0 pointer-events-none"
             style={{ background: "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(124,58,237,0.15) 0%, transparent 65%)" }} />
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold mb-5"
               style={{ background: "rgba(124,58,237,0.12)", border: "1px solid rgba(124,58,237,0.35)", color: "#c080ff" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {CPL_SUBJECTS.map(s => (
              <Link key={s.id} href={`/cpl/${s.id}`}
                    ...
                    style={{ background: "rgba(15,8,30,0.95)", border: `1px solid ${s.color}20` }}>
