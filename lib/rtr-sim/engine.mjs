...
// Indian R/T habit: "squawk double seven double zero" for 7700. Not ICAO
// phraseology, but the VALUE the student transmitted is correct and must be
// credited — the digit-repeat words expand during numeric parsing.
...
// real usage — keep it data, not logic. Most entries are what Web Speech's
// en-IN model actually returns for Indian-accented R/T: it prefers everyday
// words over phraseology ("niner"→"minor", "hotel"→"hostel", "wait"→"weight").
// Every repair target exists in the scenario phrase vocabulary; none of the
// keys do, so a repair can never corrupt a legitimate transmission.
const CONFUSIONS = {
  clime: "climb", wilko: "wilco", willco: "wilco", vilco: "wilco",
  rodger: "roger", raja: "roger", rojer: "roger",
  squark: "squawk", squak: "squawk", squack: "squawk", sqwak: "squawk",
  takeoff: "take off", pushback: "push back", startup: "start up",
  lineup: "line up",
  hostel: "hotel", tengo: "tango", juliette: "juliett",
  minor: "niner", nina: "niner", flower: "fower",
  decent: "descent", weight: "wait", taxy: "taxi",
};

// Two-token repairs, applied after the single-token pass. The recognizer
// splits compound phraseology into ordinary words ("MAYDAY" → "may day",
// "wilco" → "will co"); these stitch them back. Keys are "word word".
const BIGRAM_CONFUSIONS = {
  "may day": "mayday",
  "will co": "wilco", "wil co": "wilco",
  "a firm": "affirm",
  "down wind": "downwind",
  "run way": "runway",
...
export function tokenize(text) {
  const singles = String(text ?? "")
    .toLowerCase()
    .replace(/[^a-z0-9.\s]/g, " ")
    .split(/\s+/)
    .map((t) => t.replace(/^\.+|\.+$/g, ""))
    .filter(Boolean)
    // The recognizer fuses letters onto digits ("A1", "FL90", "27L"); expected
    // phraseology always keeps them separate tokens, so split them apart.
    .flatMap((t) => {
      const m = /^([a-z]+)(\d+)$/.exec(t) ?? /^(\d+)([a-z]+)$/.exec(t);
      return m ? [m[1], m[2]] : [t];
    })
    .flatMap((t) => (CONFUSIONS[t] ? CONFUSIONS[t].split(" ") : [t]));

  // Bigram pass: stitch split compounds back together.
  const out = [];
  for (let i = 0; i < singles.length; i++) {
    const pair = i + 1 < singles.length ? `${singles[i]} ${singles[i + 1]}` : null;
    if (pair && BIGRAM_CONFUSIONS[pair]) {
      out.push(...BIGRAM_CONFUSIONS[pair].split(" "));
      i++;
      continue;
    }
    out.push(singles[i]);
...
// A token that can join a spelled-letter run: a phonetic-alphabet word, or a
// bare single letter ("n", "d", "b"). Returns the letter, or null.
const fusibleLetter = (t) => PHONETIC[t] ?? (/^[a-z]$/.test(t) ? t : null);
...
function isNumericAt(tokens, i) {
  ...
  // "double"/"triple" open or continue a numeric group only when a digit
  // actually follows ("double seven" → 77); a bare "double" stays English.
  if (t in REPEAT_WORDS) {
    const next = tokens[i + 1];
    return next !== undefined && isNumericWord(next);
  }
  return false;
}
...
function parseNumericGroup(group) {
  ...
  for (const t of group) {
    ...
    if (t in REPEAT_WORDS) { repeat = REPEAT_WORDS[t]; continue; }
    ...
    if (d === null) continue;
    const run = d.repeat(repeat);
    repeat = 1;
    if (inFraction) fracDigits += run; else intDigits += run;
  }
...
export function normalize(text) {
  ...
  while (i < tokens.length) {
    ...
    if (fusibleLetter(tokens[i]) !== null) {
      // Fuse phonetic-alphabet runs, and ALSO runs of bare spelled letters —
      // the recognizer writes "NDB" as "n d b" and "VFR" as "v f r", which
      // must equal the fused forms "ndb"/"vfr" the phrases expect. A LONE bare
      // letter is left alone (the article "a" and pronoun "i" are English).
      let letters = "";
      let sawPhonetic = false;
      const runStart = i;
      while (i < tokens.length && fusibleLetter(tokens[i]) !== null) {
        if (PHONETIC[tokens[i]]) sawPhonetic = true;
        letters += fusibleLetter(tokens[i]);
        i++;
      }
      if (letters.length >= 2 || sawPhonetic) {
        out.push(letters);
        continue;
      }
      i = runStart;
