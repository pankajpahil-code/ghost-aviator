...
test("director×engine: every VFR circuit step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildVfrCircuit(rollWorld(s)), s);
});

test("director×engine: every SRA talkdown step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildSraApproach(rollWorld(s)), s);
});

test("director×engine: every medical emergency step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildMedicalEmergency(rollWorld(s)), s);
});

test("director×engine: every lost-pilot step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildLostPilot(rollWorld(s)), s);
});

test("director×engine: every decompression step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildEmergencyDescent(rollWorld(s)), s);
});

test("director×engine: every TCAS RA step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildTcasEvent(rollWorld(s)), s);
});

test("director×engine: every Special VFR step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildSpecialVfr(rollWorld(s)), s);
});

test("director×engine: every NDB approach step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildNdbApproach(rollWorld(s)), s);
});

test("director×engine: every PAR talkdown step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildParTalkdown(rollWorld(s)), s);
});

test("director×engine: every VFR arrival step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildVfrArrival(rollWorld(s)), s);
});

test("director×engine: every airways transit step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildAirwaysTransit(rollWorld(s)), s);
});

test("director×engine: every fuel dump step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildFuelDump(rollWorld(s)), s);
});

test("director×engine: every abandoned takeoff step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildAbandonedTakeoff(rollWorld(s)), s);
});

test("director×engine: every MSAW alert step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildMsawAlert(rollWorld(s)), s);
});

test("director×engine: every runway conditions step is fully satisfiable (30 seeds)", () => {
  for (let s = 1; s <= 30; s++) assertFlightSatisfiable(buildRunwayConditions(rollWorld(s)), s);
});
