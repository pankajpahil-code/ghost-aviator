// Ghost Tower â€” DialogueDirector: generates a flight's dialogue from a rolled
// world. v2 of what scn1.ts hardcoded â€” same step interface the UI consumes,
// but every value, trap and branch comes from WorldGen, so no two flights
// match. Every template phrase traces to the Captain's book ch13â€“15/17 and the
...
/** "1013" â†’ "one zero one three" (digit by digit â€” Rule 1, book Ch13 Â§13.4). */
export function speakDigits(value) {
  return String(value).split("").map(c => (c === "." ? "decimal" : DIGIT_WORD[Number(c)])).join(" ");
}

/** "118.35" â†’ "one one eight decimal three five". */
export const speakFreq = speakDigits;

/** Whole hundreds/thousands (Rule 2 â€” altitudes): 2500 â†’ "two thousand five hundred". */
...
/** "VT-ABC" â†’ "Victor Tango Alfa Bravo Charlie". */
export function speakCallsign(reg) {
  return reg.replace("-", "").split("").map(c => PHONETIC_WORD[c]).join(" ");
}

/** Abbreviated form after ATC shortens it: first + last two [Ch14 Â§14.4]. */
...
//           passMark, steps } â€” the exact shape GhostTower already flies.
export function buildVfrDeparture(world) {
  ...
  steps.push({
    id: "radio-check",
    cue: `First transmission of the day â€” call ${A.name} Ground for a radio check on ${A.ground}.`,
    expect: {
      slots: [
    ...
    labels: { station: "Station addressed", radiocheck: "â€œRadio checkâ€", freq: "Frequency stated" },
    chips: [
  ...
  steps.push({
    id: "taxi-request",
    cue: `Readability five both ways. Request taxi â€” type, position, VFR intentions, with information ${w.atis}.`,
    expect: {
      slots: [
  ...
  steps.push({
    id: "taxi-readback",
    cue: "Read back the taxi clearance â€” holding point, runway and QNH are mandatory.",
    expect: {
      slots: [
    ...
    corrections: { qnh: `${csShort}, negative â€” QNH ${qnhSpoken}. Read back.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "give-way",
    cue: "Traffic on the taxiway â€” respond to the instruction.",
    ...
    probes: { giveway: `${csShort}, acknowledge â€” give way to the ${w.traffic.givewayType}.` },
    callsignAlt: csShort,
  ...
  steps.push({
    ...
    corrections: { freq: `${csShort}, negative â€” Tower ${towerSpoken}.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "ready",
    cue: `Holding point ${hpSpoken}, run-up complete. TUNE ${A.tower}, then call Tower and report ready. Careful â€” "take-off" is reserved for the clearance itself.`,
    ...
    labels: { station: "Station addressed", position: "Position report", ready: "â€œReady for departureâ€" },
  ...
  if (w.events.conditionalLineup) {
    steps.push({
      id: "conditional",
      cue: `A ${w.traffic.landingType} is on short final. A conditional clearance â€” the condition must open AND close your read-back.`,
      ...
      labels: {
        ...
        behind2: "â€œBehindâ€ repeated at the end",
      },
      chips: [
      ...
      probes: {
        condition: `${csShort}, I say again â€” BEHIND the landing ${w.traffic.landingType}, line up runway ${rwySpoken} and wait, BEHIND. Read back the condition.`,
        behind2: `${csShort}, read back the full condition â€” the word "behind" closes it.`,
        lineup: `${csShort}, confirm â€” line up runway ${rwySpoken} and wait, behind the landing ${w.traffic.landingType}?`,
      },
      callsignAlt: csShort,
    });
  } else {
    steps.push({
      ...
      probes: { lineup: `${csShort}, confirm â€” line up runway ${rwySpoken} and wait?` },
      callsignAlt: csShort,
    });
  }
  ...
  steps.push({
    id: "takeoff",
    cue: "Here comes your clearance â€” runway and clearance are mandatory read-backs; the wind is information only.",
    ...
    labels: { runway: "Runway", clearance: "â€œCleared for take-offâ€" },
    chips: [`Runway ${rwySpoken},`, "cleared for take-off,", `${speakWind(w.wind)},`, "rolling,", reg],
    probes: { clearance: `${csShort}, read back â€” cleared for take-off runway ${rwySpoken}.` },
    corrections: { runway: `${csShort}, negative â€” runway ${rwySpoken}.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "wilco",
    cue: "Airborne, climbing out. Tower has an instruction â€” the correct reply is one exact word.",
  ...
  return {
    ...
    title: "VFR Departure",
    subtitle: `${A.name} Â· runway ${A.runway} Â· information ${w.atis} Â· flight #${w.seed}`,
    ...
    briefing: [
      ...
      `ATIS information ${w.atis}: runway ${A.runway}, wind ${String(w.wind.dir).padStart(3, "0")}Â° at ${w.wind.speed}${w.wind.gustTo ? ` gusting ${w.wind.gustTo}` : ""} knots, QNH ${w.qnh}.`,
...
/* ============================ IFR full flight ============================= */
// Clearance delivery â†’ push/start â†’ taxi (hold short) â†’ take-off â†’ departure
// radar (ident/squawk) â†’ [avoiding action] â†’ area control â†’ [hold + EFC] â†’
// approach (expect-ILS trap, vectors) â†’ ILS â†’ tower ([RVR]) â†’ land â†’ vacate.
// Phraseology seeds: SCENARIO_DRAFTS.md SCN-2/3/4/5 (ICAO-verified v2);
// airline Type-3 callsign â€” never abbreviated [book Ch14 Â§14.4].
export function buildIfrFlight(world) {
  ...
  steps.push({
    id: "ifr-clearance",
    cue: "Copy your clearance â€” the full route clearance is a mandatory read-back, every element.",
    ...
    corrections: { squawk: `${cs}, negative â€” squawk ${sqSpoken}. Read back.` },
  ...
  steps.push({
    ...
    cue: "Read back the push-back â€” the facing direction matters to the tug crew.",
    expect: {
      slots: [
  ...
  steps.push({
    id: "ifr-taxi",
    cue: "Request taxi, then read back everything â€” including the hold-short. Crossing an active runway without clearance is the cardinal sin.",
    ...
    corrections: { qnh: `${cs}, negative â€” QNH ${qnhSpoken}. Read back.` },
    atcAfter: `${cs}, hold short cancelled, cross runway ${speakDigits(F.holdShortRwy)}, then contact Tower ${speakFreq(A.tower)}.`,
  });

  steps.push({
    id: "ifr-freq-tower",
    cue: `Read back the crossing and the frequency â€” then TUNE ${A.tower} before you call.`,
    expect: {
      slots: [
    ...
    probes: { freq: `${cs}, confirm frequency?`, cross: `${cs}, confirm â€” cleared to cross runway ${speakDigits(F.holdShortRwy)}?` },
  });

  steps.push({
    id: "ifr-ready",
    cue: `On Tower now â€” report ready for departure at holding point ${hpSpoken}.`,
    requiresFreq: A.tower,
    expect: {
      slots: [
        { key: "station", phrases: [`${A.name.toLowerCase()} tower`, "tower"] },
    ...
    labels: { station: "Station addressed", ready: "â€œReady for departureâ€" },
    chips: [`${A.name} Tower,`, `${reg},`, `holding point ${hpSpoken},`, "ready for departure", "ready for take-off"],
  ...
  steps.push({
    ...
    chips: [`Line up runway ${rwySpoken}`, "and wait,", "cleared for take-off,", reg],
    probes: { lineup: `${cs}, confirm â€” line up runway ${rwySpoken} and wait?` },
  });

  steps.push({
    id: "ifr-takeoff",
    cue: "Your take-off clearance â€” runway and clearance are the mandatory items.",
    ...
    probes: { clearance: `${cs}, read back â€” cleared for take-off runway ${rwySpoken}.` },
    corrections: { runway: `${cs}, negative â€” runway ${rwySpoken}.` },
  ...
  steps.push({
    id: "ifr-ident",
    cue: `First call to Departure â€” passing altitude and the SID. Radar contact needs your squawk showing ${F.squawk}.`,
  ...
  if (F.events.avoidingAction) {
    steps.push({
      id: "ifr-avoiding",
      cue: "Traffic conflict â€” act on the instruction IMMEDIATELY, read back as you turn.",
      ...
      probes: { turn: `${cs}, I say again â€” turn ${F.closingSide} IMMEDIATELY heading ${speakDigits(F.avoidHdg)}. Acknowledge.` },
    ...
    steps.push({
      ...
      cue: "Handed to area control â€” read back the frequency.",
  ...
  steps.push({
    id: "ifr-control-call",
    cue: `TUNE ${F.controlFreq} and make the first call to Control â€” level and estimate for ${F.enrouteFix}.`,
  ...
  if (F.events.hold) {
    steps.push({
      id: "ifr-hold",
      cue: "Read back the hold in full â€” fix, level, and the expect-further-clearance time.",
      expect: {
        slots: [
    ...
    steps.push({
      id: "ifr-descent",
      cue: "Descent and handoff coming â€” read back everything.",
  ...
  steps.push({
    id: "ifr-app-call",
    cue: `TUNE ${D.approach} and call ${D.name} Approach with your level and information ${D.atis}. Listen for the word EXPECT â€” it is not a clearance.`,
  ...
  steps.push({
    id: "ifr-platform",
    cue: "Read back the descent and QNH. â€œExpect ILSâ€ is information â€” reading it back as a clearance is the classic trap.",
    expect: {
      slots: [
    ...
    corrections: { qnh: `${cs}, negative â€” QNH ${destQnhSpoken}. Read back.` },
  ...
  steps.push({
    id: "ifr-cleared-ils",
    cue: "NOW it is a clearance â€” read back heading, the ILS clearance and the runway.",
    expect: {
      slots: [
  ...
  steps.push({
    id: "ifr-established",
    cue: "Localizer and glideslope captured â€” report established.",
    expect: {
  ...
  steps.push({
    id: "ifr-tower-final",
    cue: `TUNE ${D.tower} and call ${D.name} Tower on the ILS.${F.events.lowVis ? " Visibility is poor â€” expect RVR." : ""}`,
  ...
  steps.push({
    id: "ifr-continue",
    cue: F.events.lowVis
      ? "Acknowledge â€” â€œcontinue approachâ€ is NOT a landing clearance. RVR may be read back or acknowledged with roger."
      : "Acknowledge â€” â€œcontinue approachâ€ is NOT a landing clearance.",
    expect: {
    ...
    probes: { cont: `${cs}, acknowledge â€” continue approach.` },
  ...
  steps.push({
    id: "ifr-land",
    cue: "The landing clearance â€” mandatory read-back.",
    expect: {
      slots: [
    ...
    labels: { runway: "Runway", clearance: "â€œCleared to landâ€" },
    chips: [`Runway ${destRwySpoken},`, "cleared to land,", "cleared for take-off,", reg],
    probes: { clearance: `${cs}, read back â€” cleared to land runway ${destRwySpoken}.` },
  ...
  return {
    ...
    title: `IFR â€” ${A.name} to ${D.name}`,
    subtitle: `${reg} Â· ${F.sid} departure Â· FL${F.cruiseFl} Â· ILS ${D.runway} at ${D.name} Â· flight #${w.seed}`,
    station: `${A.name} â†’ ${D.name}`,
    freq: A.ground,
    callsign: reg,
    ...
    briefing: [
      ...
      `This flight works five frequencies â€” Ground, Tower, Departure, Control and ${D.name} Approach/Tower. TUNE each handoff on the radio head, and set your squawk when cleared: radar contact depends on it.`,
    ],
    passMark: 50,
    steps,
  };
...
// RVR uses the whole-hundreds rule [9432 Â§2.4.3]; fifties speak naturally.
...
/* =========================== Emergency flight ============================= */
// PAN-PAN â†’ MAYDAY escalation (the Captain-approved SCN-6 shape, dynamic):
// rough engine â†’ urgency call â†’ priority handling â†’ engine failure â†’ full
// distress message â†’ squawk 7700 â†’ silence imposed on the frequency â†’
// straight-in â†’ DISTRESS TRAFFIC ENDED. Distress format per book Ch20 Â§20.2
// and the aeronautical silence signals verified against Annex 10 Vol II Â§5.3.
export function buildEmergencyFlight(world) {
  const w = world;
  const A = w.airport;
  const reg = w.callsign.reg;
  // removed: unused local (const cs = speakCallsign(reg);)
  ...
  steps.push({
    id: "em-panpan",
    cue: `Cruise at ${F.cruiseAlt.toLocaleString("en-IN")} ft, ${posPhrase.replace("of the field", `of ${A.name}`)} â€” and the engine is RUNNING ROUGH. Not critical yet. Make the urgency call: PAN-PAN Ã—3, station, callsign, nature, intentions, position and level.`,
    expect: {
      slots: [
    ...
    labels: {
      pan3: "PAN-PAN Ã— 3 (escalating to MAYDAY is never wrong)",
  ...
  steps.push({
    ...
    cue: "Read back the priority handling â€” altitude and QNH.",
    expect: {
      slots: [
  ...
  steps.push({
    id: "em-mayday",
    cue: `The engine has FAILED. Grave and imminent danger â€” this is a MAYDAY. Transmit the full distress message: MAYDAY Ã—3, station, callsign, nature, intentions, position, level and heading, persons on board. Slowly. You may not get a second chance.`,
    expect: {
      slots: [
    ...
    labels: {
      mayday3: "MAYDAY Ã— 3", nature: "Nature (engine failure)", intent: "Intentions (forced landing)",
    ...
    probes: {
      position: `${csShort}, MAYDAY acknowledged â€” say your position?`,
  ...
  steps.push({
    id: "em-7700",
    cue: "Set 7700 on the transponder â€” every radar screen in the FIR lights up â€” then acknowledge.",
    requiresSquawk: "7700",
    expect: {
      slots: [
    ...
    atcAfter: `All stations, ${A.name} Approach â€” STOP TRANSMITTING, MAYDAY. ${csShort}, field at your twelve o'clock, ${speakDigits(String(F.finalMiles))} miles, runway ${rwySpoken}, cleared to land, no traffic.`,
  });

  steps.push({
    id: "em-land-readback",
    cue: "Silence has been imposed for you â€” the frequency is yours. Read back the landing clearance and fly the aircraft.",
    expect: {
      slots: [
        { key: "runway", critical: true, value: A.runway, anchor: "runway" },
        { key: "clearance", critical: true, phrases: ["cleared to land"] },
    ...
    probes: { clearance: `${csShort}, read back â€” cleared to land runway ${rwySpoken}.` },
    atcAfter: `${csShort}, fire services alongside the runway. All stations, ${A.name} Approach â€” DISTRESS TRAFFIC ENDED. ${csShort}, well flown. Taxi with the follow-me vehicle.`,
  });

  steps.push({
    id: "em-final",
    cue: "Safely down. Close it out â€” acknowledge, and thank the crew on the ground.",
    expect: {
  ...
  return {
    id: `emergency-${w.seed}`,
    title: "Emergency â€” PAN-PAN to MAYDAY",
    subtitle: `${reg} Â· engine trouble ${posPhrase.replace("of the field", `from ${A.name}`)} Â· flight #${w.seed}`,
    ...
    briefing: [
      ...
      `The engine is about to give you the worst day of your training. The difference between urgency and distress â€” PAN-PAN versus MAYDAY â€” is the difference the examiner listens for.`,
      `The distress message format: MAYDAY Ã—3 Â· station Â· callsign Â· nature Â· intentions Â· position Â· level and heading Â· persons on board. Nature, intentions, position â€” the three things ATC needs most.`,
    ],
    passMark: 50,
    steps,
  };
...
// Comms failure inbound: calls unanswered â†’ squawk 7600 UNPROMPTED â†’ transmit
// blind â†’ receiver-only branch: "if you read, squawk ident" â†’ acknowledge by
// IDENT â†’ light-gun signals. Procedure per book Ch20 Â§20.7 and Ch19 Â§19.5.
export function buildRadioFailureFlight(world) {
  ...
  steps.push({
    id: "rf-blind",
    cue: "â€¦nothing. Two calls, no reply â€” carrier silent. Radio checks done: volume, squelch, alternate box â€” dead. Set the RADIO-FAILURE code on the transponder, then transmit BLIND: prefix, callsign, position, intentions.",
    requiresSquawk: "7600",
    gateSilent: true,
    expect: {
      slots: [
    ...
    labels: { blind: "â€œTransmitting blindâ€", position: "Position", intent: "Intentions stated" },
    chips: [
  ...
  steps.push({
    id: "rf-ident1",
    cue: "A voice, faint but readable â€” your RECEIVER works. The transmitter is dead. You cannot reply by voiceâ€¦ but radar can see you answer.",
    atcBefore: `${csShort}, ${A.name} Tower â€” transmission not received. If you read ${A.name} Tower, squawk ident.`,
    requiresIdent: true,
  ...
  steps.push({
    id: "rf-ident2",
    cue: "Acknowledge the instruction â€” the transponder is your voice now.",
    requiresIdent: true,
  ...
  steps.push({
    id: "rf-lightgun",
    cue: "Short final â€” and a STEADY GREEN light blazes from the tower. For the gradesheet: what has the tower just told you? (Answer as the read-back you cannot transmit.)",
    expect: {
      slots: [
    ...
    probes: { meaning: "Check the light-signal table â€” steady green, aircraft in flight?" },
    atcAfter: `${csShort}, light signal acknowledged â€” down and rolling. Taxi with the follow-me vehicle. Radio failure procedures complete. Well handled.`,
  });
  ...
  return {
    id: `radiofail-${w.seed}`,
    title: "Radio Failure â€” 7600",
    subtitle: `${reg} Â· inbound ${A.name} Â· comms lost Â· flight #${w.seed}`,
    ...
    briefing: [
      ...
      `Remember the ladder: squawk 7600 â†’ check the basics â†’ listen out â†’ transmit blind â†’ follow the last clearance â†’ light signals.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildVfrCircuit(world) {
  ...
  steps.push({
    id: "cct-join",
    cue: `You are ${C.joinDist} miles ${C.joinFrom} of ${A.name} at ${C.altitude.toLocaleString("en-IN")} ft. Call Tower â€” request to join for circuits, with information ${w.atis}.`,
    expect: {
      slots: [
        { key: "station", phrases: [`${A.name.toLowerCase()} tower`, "tower"] },
  ...
  steps.push({
    id: "cct-join-rb",
    cue: "Read back the joining instruction â€” circuit direction, runway and QNH are mandatory.",
  ...
  steps.push({
    id: "cct-seq",
    cue: `Acknowledge the traffic â€” you are following the ${C.trafficType}.`,
    expect: {
      slots: [
  ...
  if (C.events.wakeTurbulence) {
    steps.push({
      id: "cct-wake",
      cue: "Heavy traffic ahead â€” respond to the wake turbulence warning.",
      ...
      probes: { extend: `${csShort}, acknowledge â€” extend downwind.` },
      callsignAlt: csShort,
    });
  }
  ...
  if (C.events.goAround) {
    steps.push({
      id: "cct-goaround",
      cue: "Traffic on the runway â€” Tower says GO AROUND. The correct pilot call is â€œGOING AROUNDâ€.",
      ...
      labels: { goaround: "â€œGoing aroundâ€" },
    ...
    steps.push({
      id: "cct-fin2",
      cue: "Turning final â€” this time for landing.",
    ...
    steps.push({
      id: "cct-tgo",
      cue: "Touch and go clearance â€” read back runway and clearance.",
      ...
      labels: { runway: "Runway", clearance: "â€œCleared touch and goâ€" },
      chips: [`Runway ${rwySpoken},`, "cleared touch and go,", "cleared to land,", reg],
      probes: { clearance: `${csShort}, read back â€” cleared touch and go runway ${rwySpoken}.` },
      callsignAlt: csShort,
    });
  ...
  steps.push({
    id: "cct-land",
    cue: "Full-stop landing clearance â€” runway and clearance are mandatory read-backs.",
  ...
  return {
    ...
    title: "VFR Circuit",
    subtitle: `${A.name} Â· ${C.direction}-hand Â· runway ${A.runway} Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `ATIS information ${w.atis}: runway ${A.runway}, wind ${String(w.wind.dir).padStart(3, "0")}Â° at ${w.wind.speed}${w.wind.gustTo ? ` gusting ${w.wind.gustTo}` : ""} knots, QNH ${w.qnh}.`,
      `Practice the complete circuit pattern: join, downwind, base, final, and respond to whatever Tower throws at you â€” go-arounds, touch-and-goes, wake turbulence.`,
    ],
    passMark: 50,
    steps,
  };
...
/* =================== Surveillance Radar Approach â€” Â§7.6 =================== */
export function buildSraApproach(world) {
  ...
  // removed: unused local (const qnhSpoken = speakDigits(w.qnh);)
  const steps = [];
  ...
  steps.push({
    id: "sra-setup",
    cue: "Read back the key items: runway and altitude to maintain. This is an SRA â€” ATC talks you down.",
  ...
  steps.push({
    id: "sra-turn-final",
    cue: "Turn onto final â€” read back the heading.",
  ...
  steps.push({
    id: "sra-descend",
    cue: "Commence descent â€” acknowledge.",
    ...
    probes: { desc: `${csShort}, acknowledge â€” commence descent.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "sra-gear",
    cue: "Check your gear â€” acknowledge the call.",
  ...
  steps.push({
    id: "sra-5nm",
    cue: "Five miles â€” acknowledge the advisory altitude.",
  ...
  if (S.events.azimuthCorrection) {
    ...
    steps.push({
      id: "sra-azimuth",
      cue: `Going ${corrDir} of track â€” read back the corrected heading.`,
  ...
  steps.push({
    id: "sra-land",
    cue: "Landing clearance during the SRA â€” mandatory read-back.",
    ...
    labels: { runway: "Runway", clearance: "â€œCleared to landâ€" },
    chips: [`Runway ${rwySpoken},`, "cleared to land,", reg],
    probes: { clearance: `${csShort}, read back â€” cleared to land.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "sra-no-ack",
    cue: "ATC says â€œdo not acknowledge further transmissionsâ€. From here you just listen â€” report runway in sight when you see the runway.",
  ...
  return {
    id: `sra-${w.seed}`,
    title: "SRA Talkdown",
    subtitle: `${A.name} Â· runway ${A.runway} Â· surveillance radar approach Â· flight #${w.seed}`,
    station: `${A.name} Approach`,
    freq: A.approach,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `This is a Surveillance Radar Approach (SRA) â€” ATC talks you down with distance, advisory altitude and azimuth information. You acknowledge each transmission until told "do not acknowledge further transmissions".`,
      `Key discipline: gear down and locked, watch advisory altitudes, report runway in sight. The approach terminates at ${S.termination} mile${S.termination > 1 ? "s" : ""} from touchdown â€” you fly the rest visually.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildMedicalEmergency(world) {
  ...
  steps.push({
    id: "med-panpan",
    cue: `A passenger has a ${M.condition}. This is URGENCY, not distress â€” PAN-PAN Ã—3, station, callsign, nature, position, level, intentions, persons on board.`,
    expect: {
      slots: [
    ...
    labels: {
      pan3: "PAN-PAN Ã— 3", nature: "Nature of urgency", intent: "Intentions",
  ...
  steps.push({
    id: "med-readback",
    cue: "Read back the clearance â€” runway and QNH.",
    expect: {
      slots: [
  ...
  steps.push({
    id: "med-land",
    cue: "Landing clearance â€” mandatory read-back.",
  ...
  return {
    id: `medical-${w.seed}`,
    title: "Medical Emergency â€” PAN-PAN",
    subtitle: `${reg} Â· passenger ${M.condition} Â· ${M.distance} miles ${M.direction} Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `Your passenger has a ${M.condition}. This is URGENCY (PAN-PAN), not distress (MAYDAY) â€” the aircraft itself is fine. The difference matters for the examiner.`,
      `PAN-PAN Ã—3 â†’ station â†’ callsign â†’ nature â†’ position â†’ level â†’ intentions â†’ persons on board. Request PRIORITY landing, not a forced landing.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildLostPilot(world) {
  ...
  steps.push({
    id: "lost-panpan",
    cue: `Above cloud at ${L.altitude.toLocaleString("en-IN")} ft, heading ${L.lastHeading}Â°, unsure of your position. Call the nearest station â€” PAN-PAN Ã—3, type, level, heading, and ask for help.`,
    expect: {
      slots: [
        { key: "pan3", critical: true, phrases: ["pan pan pan pan pan pan"] },
    ...
    labels: {
      pan3: "PAN-PAN Ã— 3", lost: "Uncertain position",
  ...
  steps.push({
    id: "lost-qdm",
    cue: "Tower has a DF bearing for you â€” the QDM. Read back: â€œClass [letter] [bearing]â€. Class A is accurate to Â±2Â°, Class B to Â±5Â°.",
  ...
  steps.push({
    id: "lost-land",
    cue: "Straight-in clearance â€” read back runway and clearance.",
    expect: {
      slots: [
        { key: "runway", critical: true, value: A.runway, anchor: "runway" },
        { key: "clearance", critical: true, phrases: ["cleared to land"] },
  ...
  return {
    ...
    title: "Lost Pilot â€” Uncertain Position",
    subtitle: `${reg} Â· above cloud Â· heading ${L.lastHeading}Â° Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      `You are ${reg}, a ${w.aircraftType}, at ${L.altitude.toLocaleString("en-IN")} ft, heading ${L.lastHeading}Â°, above cloud, and you are NOT SURE WHERE YOU ARE.`,
      `This is urgency â€” PAN-PAN, not MAYDAY. The aircraft is flying fine; you are the problem. Swallow the pride, make the call, and let ATC find you.`,
...
/* ================= Emergency Descent (Decompression) â€” Â§9.4 ================ */
export function buildEmergencyDescent(world) {
  ...
  steps.push({
    id: "dec-mayday",
    cue: `FL${DC.cruiseFl}, ${DC.distance} miles ${DC.direction} of ${DC.fix} â€” BANG. Cabin altitude warning, masks deployed. This is MAYDAY. Transmit: MAYDAY Ã—3, station, callsign, nature (decompression), intentions (emergency descent to FL${DC.targetFl}), position.`,
    expect: {
      slots: [
        { key: "mayday3", critical: true, phrases: ["mayday mayday mayday"] },
    ...
    labels: {
      mayday3: "MAYDAY Ã— 3", nature: "Nature (decompression)",
  ...
  steps.push({
    id: "dec-souls",
    cue: "Report souls on board and endurance â€” ATC needs it for SAR planning.",
    expect: {
      slots: [
  ...
  return {
    ...
    title: "Emergency Descent â€” Decompression",
    subtitle: `${reg} Â· FL${DC.cruiseFl} Â· ${DC.fix} Â· rapid decompression Â· flight #${w.seed}`,
    ...
    briefing: [
      ...
      `The cabin altitude warning just fired. Oxygen masks are down. You need FL100 NOW. This is the most time-critical emergency â€” initiate the descent FIRST, then talk.`,
      `MAYDAY Ã—3 â†’ station â†’ callsign â†’ nature (decompression) â†’ intentions (emergency descent to FL${DC.targetFl}) â†’ position. Then ATC broadcasts the warning to all traffic.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildTcasEvent(world) {
  ...
  steps.push({
    id: "tcas-cruise",
    cue: `Cruising at FL${T.currentFl}. ATC gives you a level change â€” but just as you are about to comply, the TCAS fires an RA: ${T.raDirection.toUpperCase()}. You MUST follow the RA and tell ATC you CANNOT comply.`,
    ...
    labels: { unable: "â€œUNABLEâ€", tcas: "TCAS RA reported" },
  ...
  steps.push({
    id: "tcas-clear",
    cue: `RA resolved â€” you are CLEAR OF CONFLICT. Report: "clear of conflict, returning to clearance, now maintaining FL${T.currentFl}".`,
    expect: {
      slots: [
    ...
    labels: { clear: "â€œClear of conflictâ€", return: "Returning to clearance" },
    chips: [
  ...
  return {
    id: `tcas-${w.seed}`,
    title: "TCAS RA â€” Resolution Advisory",
    subtitle: `${reg} Â· FL${T.currentFl} Â· ${T.raDirection} RA Â· ${T.conflictType} at ${T.conflictClock} o'clock Â· flight #${w.seed}`,
    station: `${A.name} Control`,
    freq: A.control,
    callsign: reg,
    aircraft: "Ghostair jet",
    briefing: [
      ...
      `When TCAS fires a Resolution Advisory, you FOLLOW IT â€” it overrides ATC. Say "UNABLE, TCAS RA" if ATC gives a conflicting instruction. ATC will not modify your flight path until you report clear.`,
...
export function buildSpecialVfr(world) {
  ...
  steps.push({
    id: "svfr-request",
    cue: `Visibility ${SV.visibility} metres, cloud base ${SV.cloudBase} ft â€” below VMC for the zone. Request Special VFR from Tower.`,
    expect: {
      slots: [
        { key: "station", phrases: [`${A.name.toLowerCase()} tower`, "tower"] },
  ...
  steps.push({
    id: "svfr-clearance",
    cue: "Read back the Special VFR clearance â€” route, altitude, and QNH.",
  ...
  return {
    id: `svfr-${w.seed}`,
    title: "Special VFR",
    subtitle: `${reg} Â· ${A.name} CTR Â· visibility ${SV.visibility}m Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `Visibility ${SV.visibility} metres, cloud base ${SV.cloudBase} ft â€” below VMC for the control zone. You need SPECIAL VFR to enter.`,
...
export function buildNdbApproach(world) {
  ...
  steps.push({
    id: "ndb-cleared",
    cue: "Read back the approach clearance â€” runway, altitude, and QNH.",
  ...
  steps.push({
    id: "ndb-outbound",
    cue: `Passing the ${N.beaconId} beacon outbound â€” report.`,
    expect: {
      slots: [
  ...
  steps.push({
    id: "ndb-land",
    cue: "Landing clearance from Tower â€” mandatory read-back.",
    atcBefore: `${csShort}, ${speakWind(w.wind)}, runway ${rwySpoken}, cleared to land.`,
    expect: {
      slots: [
        { key: "runway", critical: true, value: A.runway, anchor: "runway" },
  ...
  return {
    id: `ndb-${w.seed}`,
    title: "NDB Approach",
    subtitle: `${A.name} Â· ${N.beaconId} Â· runway ${A.runway} Â· flight #${w.seed}`,
    station: `${A.name} Approach`,
    freq: A.approach,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `Request an NDB approach using the ${N.beaconId} beacon for runway ${A.runway}. The inbound track is ${N.inboundTrack}Â°.`,
...
export function buildParTalkdown(world) {
  ...
  steps.push({
    id: "par-correction",
    cue: "ATC gives glidepath AND azimuth information â€” this is what makes PAR different from SRA. Acknowledge.",
  ...
  steps.push({
    id: "par-no-ack",
    cue: "â€œDo not acknowledge further transmissionsâ€ â€” from here, just listen and fly. Report runway in sight.",
  ...
  return {
    id: `par-${w.seed}`,
    title: "PAR Talkdown",
    subtitle: `${A.name} Â· runway ${A.runway} Â· precision approach radar Â· flight #${w.seed}`,
    station: `${A.name} Approach`,
    freq: A.approach,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `This is a Precision Approach Radar (PAR) â€” like SRA, but ATC also tells you whether you are above or below the glidepath, not just left/right of track.`,
...
export function buildVfrArrival(world) {
  ...
  steps.push({
    id: "arr-cleared",
    cue: "Read back the clearance â€” reporting point, altitude, and QNH.",
  ...
  return {
    ...
    title: "VFR Arrival",
    subtitle: `${reg} Â· inbound ${A.name} Â· ${VA.joinMethod} Â· flight #${w.seed}`,
    station: `${A.name} Approach`,
    freq: A.approach,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `This is the standard VFR inbound sequence â€” the complement to Scenario 1 (VFR Departure).`,
    ],
    passMark: 50,
    steps,
  };
...
/* =================== Airways Transit â€” Â§8.4-8.6 + Â§11.1 =================== */
export function buildAirwaysTransit(world) {
  ...
  if (AW.events.revisedEstimate) {
    ...
    steps.push({
      id: "aw-revised",
      cue: `Headwinds stronger than planned â€” revised estimate for ${AW.nextFix} is ${revised} minutes. Report.`,
      expect: {
        slots: [
  ...
  return {
    ...
    subtitle: `${reg} Â· ${AW.airway} Â· ${AW.entryFix}â†’${AW.exitFix} Â· FL${AW.fl} Â· flight #${w.seed}`,
    station: `${A.name} Control`,
    freq: A.control,
    callsign: reg,
    aircraft: "Ghostair jet",
    briefing: [
      ...
      `Position report format: callsign â†’ position â†’ level â†’ next position â†’ estimate. Get the order right â€” examiners love this one.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildFuelDump(world) {
  ...
  steps.push({
    id: "fd-panpan",
    cue: `${FD.reason} â€” you need to return but you are too heavy to land. Request fuel dump. This is URGENCY (PAN-PAN).`,
    expect: {
      slots: [
        { key: "pan3", critical: true, phrases: ["pan pan pan pan pan pan"] },
    ...
    labels: { pan3: "PAN-PAN Ã— 3", nature: "Nature", request: "Request fuel dump", position: "Position" },
    chips: [
  ...
  steps.push({
    id: "fd-cleared",
    cue: "Read back the fuel dump clearance â€” heading and altitude.",
  ...
  return {
    id: `fuel-dump-${w.seed}`,
    title: "Fuel Dump",
    subtitle: `${reg} Â· ${FD.reason} Â· ${FD.fix} Â· flight #${w.seed}`,
    station: `${A.name} Control`,
    freq: A.control,
    callsign: reg,
    aircraft: "Ghostair jet",
    briefing: [
      ...
      `PAN-PAN (not MAYDAY â€” the aircraft is flyable). Request fuel dump. ATC will give you a heading and altitude, then broadcast a warning to all traffic.`,
    ],
    passMark: 50,
    steps,
  };
...
export function buildAbandonedTakeoff(world) {
  ...
  steps.push({
    ...
    labels: { runway: "Runway", clearance: "â€œCleared for takeoffâ€" },
    chips: [`Runway ${rwySpoken},`, "cleared for takeoff,", "cleared to land,", reg],
    probes: { clearance: `${csShort}, read back â€” cleared for takeoff.` },
    callsignAlt: csShort,
  });

  steps.push({
    id: "at-stopping",
    cue: `Rolling â€” ${AT.reason}! The ICAO word is "STOPPING". Say it NOW.`,
    expect: {
      slots: [
  ...
  steps.push({
    id: "at-hold",
    cue: "Acknowledge â€” hold position.",
    expect: {
  ...
  steps.push({
    id: "at-vacate",
    cue: "Emergency services clear. Vacate when able â€” read back.",
  ...
  return {
    ...
    subtitle: `${reg} Â· runway ${A.runway} Â· ${AT.reason} Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `Something will go wrong during the takeoff roll. The ICAO standard word for abandoning the takeoff is "STOPPING" â€” not "aborting", not "rejecting". Just "STOPPING".`,
...
export function buildMsawAlert(world) {
  ...
  steps.push({
    id: "msaw-alert",
    cue: "ATC shouts PULL UP â€” this is a Minimum Safe Altitude Warning. RESPOND IMMEDIATELY.",
    ...
    labels: { pullup: "â€œPulling upâ€", alt: "Climbing to" },
  ...
  steps.push({
    id: "msaw-ack",
    cue: "Acknowledge â€” maintain the altitude.",
    expect: {
      slots: [
  ...
  return {
    id: `msaw-${w.seed}`,
    title: "MSAW â€” Terrain Alert",
    subtitle: `${reg} Â· ${M.terrain} ${M.direction} Â· PULL UP Â· flight #${w.seed}`,
    station: `${A.name} Control`,
    freq: A.control,
    callsign: reg,
    aircraft: "Ghostair jet",
    briefing: [
      ...
      `ATC's Minimum Safe Altitude Warning (MSAW) system has triggered. When you hear "PULL UP", you climb IMMEDIATELY â€” acknowledge and comply, no questions asked.`,
...
export function buildRunwayConditions(world) {
  ...
  steps.push({
    id: "rwy-initial",
    cue: "Call Tower for landing. Winter conditions â€” expect a runway condition report.",
    expect: {
      slots: [
        { key: "station", phrases: [`${A.name.toLowerCase()} tower`, "tower"] },
  ...
  steps.push({
    id: "rwy-condition",
    cue: "Tower passes runway conditions. Read back the braking action â€” this is mandatory.",
  ...
  steps.push({
    id: "rwy-land",
    cue: "Landing clearance â€” read back runway and clearance.",
  ...
  return {
    ...
    subtitle: `${reg} Â· ${A.name} Â· ${RC.contaminant} Â· braking ${RC.brakingAction} Â· flight #${w.seed}`,
    station: `${A.name} Tower`,
    freq: A.tower,
    callsign: reg,
    aircraft: w.aircraftType,
    briefing: [
      ...
      `The runway is contaminated: ${RC.contaminant}, ${RC.coverage}% coverage, ${RC.depth}mm depth. Tower will pass braking action â€” READ IT BACK.`,
      `After landing, Tower may ask for YOUR observed braking action. Report what you actually experienced on the rollout â€” this helps the next pilot.`,
    ],
    passMark: 50,
    steps,
  };
