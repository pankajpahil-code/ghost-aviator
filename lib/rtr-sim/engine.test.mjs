...
// --- Indian-accent recognizer repairs (en-IN Web Speech real-world output) ---

test("accent repairs: everyday-word substitutions map back to phraseology", () => {
  // en-IN prefers common words over R/T words; each key was observed or is a
  // near-certain confusion with NO legitimate use in the phrase vocabulary.
  assert.equal(norm("line up runway 27 and weight"), "line up runway 27 and wait");
  assert.equal(norm("request startup"), "request start up");
  assert.equal(norm("ready for departure willco"), "ready for departure wilco");
  assert.equal(norm("minor flower"), "94");            // niner fower
  assert.equal(norm("hostel golf"), "hg");             // hotel golf fuses
  assert.equal(norm("request taxy"), "request taxi");
});

test("accent repairs: split compounds stitch back together (bigrams)", () => {
  assert.equal(norm("may day may day may day"), "mayday mayday mayday");
  assert.equal(norm("will co"), "wilco");
  assert.equal(norm("a firm"), "affirm");
  assert.equal(norm("join down wind"), "join downwind");
  assert.equal(norm("run way vacated"), "runway vacated");
});

test("letter-digit fusions split apart (A1, FL90)", () => {
  assert.equal(norm("taxi to holding point A1"), "taxi to holding point a 1");
  assert.equal(norm("climbing FL90"), "climbing fl 90");
});

test('digit-repeat habit credits the value ("double seven double zero")', () => {
  assert.equal(norm("squawk double seven double zero"), "squawk 7700");
  assert.equal(norm("triple two"), "222");
  // A bare "double" with no digit after it stays English.
  assert.equal(norm("double trouble"), "double trouble");
});

test("bare spelled letters fuse like phonetic runs (VFR, NDB), articles survive", () => {
  assert.equal(norm("request special v f r"), "request special vfr");
  assert.equal(norm("request n d b approach"), "request ndb approach");
  // The article "a" and pronoun "i" are English words, not spelled letters.
  assert.equal(norm("request a radio check"), "request a radio check");
  assert.equal(norm("i say again"), "i say again");
});

test("accent-repaired transmission scores clean end to end", () => {
  const expect = {
    slots: [
      { key: "lineup", critical: true, phrases: ["line up runway 27 and wait", "line up and wait"] },
    ],
    callsign: "VT-ABC",
  };
  const r = scoreTransmission(
    expect,
    "line up runway 27 and weight, victor tango alpha bravo charlie",
  );
  assert.equal(r.slots[0].status, "ok");
