...
export const SCENARIOS = [
  ...
  },
  {
    ...
    options: {
      // This option previously read "the paperwork is signed, so the matter is
      // closed whatever I think" — which is DEFERENCE, not anti-authority, and
      // near-duplicated this scenario's own resignation option. Worse: p3 and
      // p6 are the module's only consistency pair, so a student who genuinely
      // resents procedure in both would have been marked INCONSISTENT. Now it
      // resents the reporting system, parallel to p6.
      "anti-authority": "Writing it up again just feeds a paperwork machine that grounds aeroplanes over nothing. I would leave it.",
  ...
  },
  {
    ...
    options: {
      // Sharpened from "seniority does not make him right, I would tell him he
      // is doing it wrong". Speaking up about a wrong set-up is CORRECT crew
      // behaviour, so the old wording would have scored a student's good
      // assertiveness as a hazardous attitude. The hazard is the contempt for
      // rank, not the raising of the concern.
      "anti-authority": "I have never had much time for the idea that the left seat knows better, and I would let him know it.",
