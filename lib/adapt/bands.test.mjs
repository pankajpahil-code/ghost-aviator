...
test("a bottom and a top stanine land in the extreme bands", () => {
  // Band keys changed 2026-08-12 when the real six-band names became known from
  // a candidate's report. The assertion is the same: the extremes must be the
  // extremes, whatever they are called.
  assert.equal(reportLine(1).band.key, COLOUR_BANDS[0].key);
  assert.equal(reportLine(9).band.key, COLOUR_BANDS[COLOUR_BANDS.length - 1].key);
});

test("the six bands are the ones the real report names", () => {
  // From a candidate reading his own report: "how excellent, good, reasonable,
  // need attention, support, and need to learn". Worst-first here, so the list
  // reads the same way the sten scale climbs.
  assert.deepEqual(
    COLOUR_BANDS.map((b) => b.label),
    ["Need to learn", "Support", "Need attention", "Reasonable", "Good", "Excellent"]
  );
});

test("green is the best band and red the worst, as the candidate described", () => {
  assert.equal(COLOUR_BANDS[COLOUR_BANDS.length - 1].hex, "#22c55e", "the top band should be green");
  assert.equal(COLOUR_BANDS[0].hex, "#ef4444", "the bottom band should be red");
});
