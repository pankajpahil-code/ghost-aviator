...
test("a session builds every registered module with its full paper", () => {
...
test("no module repeats a question within its own paper", () => {
  for (let s = 1; s <= 40; s++) {
    for (const mod of buildSession(s * 31337).modules) {
      assert.equal(new Set(mod.items.map((i) => i.stem)).size, mod.items.length, `${mod.id} seed ${s}: repeated stem`);
    }
  }
});
...
test("the composite reflects both modules", () => {
  const session = buildSession(555);
  const results = session.modules.map((mod) => scoreModule(mod, allCorrect(mod), 600));
