...
/**
 * Learning curves kept SEPARATE per difficulty setting.
 *
 * Added 2026-08-16 with the difficulty selector. A trend drawn across mixed
 * settings is worse than no trend: a student who practises three times on Hard
 * and then once on Gentle sees a jump and reads it as progress, when all that
 * changed was the clock. The criterion ladder is identical at every setting on
 * purpose (see DIFFICULTY in session.mjs), which is exactly WHY the scores are
 * not comparable across settings and must not be pooled.
 *
 * `attempts` is `[{ score, difficulty }]`, oldest first. Entries with no
 * difficulty recorded — every sitting taken before this shipped — are grouped
 * under "standard", because that is the only setting that existed then. That
 * is a stated assumption, not a guess dressed as data.
 *
 * Returns `{ [difficulty]: learningFor(scores) }`, only for settings actually
 * sat. A setting with no attempts is absent rather than present-and-empty.
 */
export function learningByDifficulty(attempts, fallback = "standard") {
  const buckets = new Map();
  for (const a of attempts ?? []) {
    if (!a || !Number.isFinite(a.score)) continue;
    const key = a.difficulty ?? fallback;
    if (!buckets.has(key)) buckets.set(key, []);
    buckets.get(key).push(a.score);
  }
  const out = {};
  for (const [key, scores] of buckets) out[key] = learningFor(scores);
  return out;
}

/**
 * The setting a student has actually practised most, or null when they have
 * sat none. Used to decide which curve to lead the dashboard with, so the
 * headline is the one with the most evidence behind it rather than whichever
 * happens to sort first.
 */
export function mostPractised(byDifficulty) {
  let best = null;
  for (const [key, l] of Object.entries(byDifficulty ?? {})) {
    if (!best || l.sittings > byDifficulty[best].sittings) best = key;
  }
  return best;
}
