...
function dividedDetail(wrapper) {
  ...
  return {
    ...
    // The debrief and the latency, added 2026-08-16. Counts and medians only —
    // never a question, never a pick, never a per-event time. Which questions a
    // student got wrong stays on their device like everything else here; what
    // leaves is the shape of the run, which is what shows whether a generator
    // is pitched wrong.
    // Counts only, same rule as everything else here: how many were spotted,
    // missed, mis-called or called at nothing — never which ones or when.
    sightings: r.sightings
      ? {
          total: r.sightings.total,
          correct: r.sightings.correct,
          missed: r.sightings.missed,
          misidentified: r.sightings.misidentified,
          falseReports: r.sightings.falseReports,
          accuracy: pct(r.sightings.accuracy * 100),
        }
      : null,
    recall: r.recall
      ? {
          correct: r.recall.correct,
          total: r.recall.total,
          wrong: r.recall.wrong,
          unanswered: r.recall.unanswered,
          accuracy: pct(r.recall.accuracy * 100),
        }
      : null,
    responseTime: {
      radio: latency(r.responseTime?.radio),
      interruptions: latency(r.responseTime?.interruptions),
    },
  };
}

/** Median latency, stored to a tenth of a second. Null stays null — never a zero. */
function latency(l) {
  if (!l || !Number.isFinite(l.medianSec)) return null;
  return {
    medianSec: Math.round(l.medianSec * 10) / 10,
    medianWindowUsed: pct(l.medianWindowUsed * 100),
    n: l.n,
  };
}
...
export function buildResultRows(seed, results, userId) {
  ...
  return (results ?? [])
    ...
    .map((r) => {
      ...
      // The setting rides inside `detail` rather than in a new column: it needs
      // no migration, it is available immediately to every already-deployed
      // reader, and a score whose setting is unknown is a score that cannot be
      // trended honestly. Absent on rows written before 2026-08-16, which the
      // trend treats as "standard" and says so.
      const withDifficulty = (d) => ({ ...d, difficulty: r.difficulty ?? null, difficultyApplies: Boolean(r.difficultyApplies) });

      if (r.kind === "knowledge") {
        return { ...scored, headline_pct: pct((r.correct / r.total) * 100), detail: withDifficulty(knowledgeDetail(r)) };
      }
      if (r.kind === "psychomotor") {
        return { ...scored, headline_pct: pct(r.cancellation), detail: withDifficulty(trackingDetail(r)), input_class: r.inputClass ?? null };
      }
      if (r.kind === "divided-attention") {
        return { ...scored, headline_pct: pct(r.composite), detail: withDifficulty(dividedDetail(r)) };
      }
      return scored;
    });
}
 ...
 *
 * Lives here, in the pure module, because BOTH the client that saves and the
 * probe that verifies the behaviour need it. It was written out twice at first
 * — once in results.ts and once in the probe — which meant the probe could go
 * on passing after the real predicate had changed. A check that can drift away
 * from the thing it checks is not a check.
 *
 * Why the condition matters at all: the site deploys from a git push and the
 * table is created by hand, so whichever happens second leaves a window. In
 * that window a signed-in student must be told "not switched on yet", not shown
 * a Postgres schema-cache message that reads as though their result was lost.
 *
 * PostgREST reports it differently across versions, so both the Postgres code
 * for an undefined table and PostgREST's own message are matched.
 */
